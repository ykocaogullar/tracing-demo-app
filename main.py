...
# deepeval's OpenAI client is a drop-in replacement for `openai.OpenAI`: every
# `chat.completions.create(...)` call becomes an `llm` span automatically, with
# the prompt messages, model, output, and token usage captured. The
# `CONFIDENT_API_KEY` environment variable routes these traces to Confident AI.
...
def answer(query: str) -> str:
    ...
    update_current_trace(input=query, output=result, tags=["rag", "support-qa"])
    return result
